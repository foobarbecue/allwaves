...
export async function drawGeodataForDay(
  ...
  if (!seshGeodataCache.hasOwnProperty(seshDate)) {
    ...
    seshData.locations.map((datum) => {
      ...
      seshDataByTag[datum.tagId].push([
        datum.tagPosition.latitude,
        datum.tagPosition.longitude,
      ]);
      timeStampsByTag[datum.tagId].push(
        seshData.absTimestamps.slice(-1)[0] +
          datum.timestamp -
          seshData.locations[0].timestamp,
      );
    });
    seshGeodataCache[seshDate] = seshDataByTag;
    seshTimestampCache[seshDate] = timeStampsByTag;
  }
  await setMapContents(
    seshGeodataCache[seshDate],
    seshTimestampCache[seshDate],
    window.waveMap,
  );
}
    ...
    if (playlist.snippet.title.includes("surfing")) {
      ...
      for (const vid of playlist_itemlist.items) {
        ...
        vid_li_a.onclick = async () => {
          ...
          document.querySelector("#wave-video-title").textContent =
            `Playing video: ${vid.snippet.title}`;
          window.player.loadVideoById(vid.snippet.resourceId.videoId);
          await drawGeodataForDay(
            seshDate,
            seshGeodataCache,
            seshTimestampCache,
          );
...
const formatDescription = async (playlist_vid) => {
  ...
  waves_filtered.map((wave) => {
    ...
    wave_li_a.onclick = async () => {
      ...
      document.querySelector("#wave-video-title").textContent =
        `Playing video: ${playlist_vid.snippet.title}`;
      window.player.loadVideoById(
        playlist_vid.snippet.resourceId.videoId,
        tot_secs,
      );
