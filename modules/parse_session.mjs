...
export async function parseSession(sessionReader) {
  ...
  assert(
    enc.decode(r.read(4)) === "SOLO",
    'Invalid session file. Expected file content to start with "SOLO"',
  );
  r.read(12);
  ...
  while (!r.isEnd()) {
    assert(
      r.read(1)[0] === TYPE_PREFIX,
      "Unexpected file content. Expected type prefix, but got something else",
    );
